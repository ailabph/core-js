...
type AnalyzeLogsInvolvementSummary = {
    blockNumber:number,
    has_bnb_usd:boolean,
    has_token:boolean,
    has_token_dex:boolean,
}
export { AnalyzeLogsInvolvementSummary }
//endregion TYPES
...
export class eth_receipt_logs_tools{
    ...
    private static log(msg:string,method:string,end:boolean=false,force_display:boolean=false):void{
        if(config.getConfig().verbose_log || force_display){
            ...
            if(end) console.log(`receipt_logs_tools|${method}|${tools.LINE}`);
        }
    }
    ...
    public static async analyzeLogsInvolvement(logs:Log[]){
        ...
        for(const log of logs as Log[]){
            this.log(`${++scan_count}/${total_logs} block ${log.blockNumber} hash ...${tools.lastSubstring(log.transactionHash,6)} logIndex ${log.logIndex}`,method);
