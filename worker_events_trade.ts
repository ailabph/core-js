...
export class worker_events_trade{
    ...
    public static async run(){
        ...
        try{
            ...
            await dexLogs.list(
                " WHERE id>:last_id AND time_processed_price>:zero AND has_token_dex=:y AND time_processed_events IS NULL ",
                {last_id:this.last_id,zero:0,y:"y"},
                ` ORDER BY blockTime ASC, logIndex ASC LIMIT ${this.getBatch()} `);
            if(dexLogs.count() > 0){
            ...
            for(const log of dexLogs._dataList as eth_receipt_logs[]){
                ...
                if(swapLog){
                    ...
                    newEvent.fromDecimal = assert.naturalNumber(swapSummary.from.contractInfo.decimals);
                    ...
                    newEvent.toDecimal = assert.naturalNumber( swapSummary.to.contractInfo.decimals,"swapSummary.to.contractInfo.decimals");
