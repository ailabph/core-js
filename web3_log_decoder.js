...
exports.web3_log_decoder = void 0;
...
//endregion
class web3_log_decoder {
    static getDefaultBaseType() {
        return { ContractInfo: {
                address: "",
                name: "",
                symbol: "",
    ...
    static async decodeLog(log) {
        ...
        let contractMetaData = await eth_worker_1.eth_worker.getContractMetaData(log.address);
    ...
    static async getTransferLog(log) {
    ...
    static async getApprovalLog(log) {
        let decodedLog = await this.decodeLog(log);
        if (!decodedLog)
            return false;
    ...
    static async getWithdrawalLog(log) {
        let decodedLog = await this.decodeLog(log);
        if (!decodedLog)
            return false;
    ...
    static async getSyncLog(log) {
        let decodedLog = false;
        try {
            decodedLog = await this.decodeLog(log);
        } // suppress error due to buffer overrun
        catch (e) { }
        if (!decodedLog)
            return false;
    ...
    static async getDepositLog(log) {
        let decodedLog = await this.decodeLog(log);
        if (!decodedLog)
            return false;
...
exports.web3_log_decoder = web3_log_decoder;
//# sourceMappingURL=web3_log_decoder.js.map