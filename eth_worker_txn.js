...
class eth_worker_txn {
    static run() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!eth_worker_txn.hasRun) {
                ...
                console.log(`running txn worker on env:${ailab_core_1.config.getEnv()} rpc:${ailab_core_1.eth_config.getRPCUrl()}`);
            }
            yield ailab_core_1.connection.startTransaction();
            try {
                let lastBlockProcessed = -1;
                const latestBlock = yield ailab_core_1.eth_worker.getLatestBlock();
                ...
                for (const block of unprocessedBlock._dataList) {
                    ...
                    const transactions = yield ailab_core_1.eth_worker.getTxnByBlockNumberWeb3(block.blockNumber);
                    ...
                    for (const transaction of transactions.transactions) {
                        ...
                        // tag txn if involved
                        yield ailab_core_1.eth_worker.identifyInvolvement(newTxn);
                    ...
                    block.time_txn_retrieved = ailab_core_1.tools.getCurrentTimeStamp();
                    yield block.save();
            ...
            yield ailab_core_1.tools.sleep(eth_worker_txn.waitMs());
    ...
    static getBatchLimit() {
        const batch_limit = ailab_core_1.config.getCustomOption("worker_txn_batch");
